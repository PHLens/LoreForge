...
- paper note shape under `Sources/Papers/<citekey>.md`
...
- `loreforge-domain` for domain orientation, Source/Space writes, and
...
1. **Resolve context**
   ...
   - If the paper cannot be resolved to one Zotero item, stop and ask the user
     ...
     copy PDFs into the vault, or fall back to `Sources/Raw/`.
   - The canonical note path is
     `Sources/Papers/<citekey>.md`. Inspect an existing note at that path
     before writing. Inspect `00_System/card-domains.md`, relevant
     `Cards/<domain>/`, `Atlas/`, and `Spaces/` pages only for read-only
     orientation or downstream handoff.
   - If `Sources/Papers/` does not exist, stop and ask for the wiki layout to
     be initialized or repaired. Do not create a new paper-note directory from
     this workflow unless the caller explicitly approved initialization.
...
2. **Read Zotero raw files**
   - Treat Zotero-managed PDF files, snapshots, and attachment directories as
     ...
     relocate them.
   - Do not create or reorganize wiki-local Zotero bundles, `Sources/Raw/`, or any
...
3. **Route**
   - Paper notes live at the wiki root under `Sources/Papers/`.
...
4. **Write the paper note**
   - Write Markdown note files only inside
     `Sources/Papers/`. Creating `<citekey>.md` is allowed when no note
     exists; otherwise update the existing same-citekey note unless the user
...
`Sources/Papers/`. Notes should preserve Zotero or importer
...
Do not use `Sources/Raw/` for paper PDFs or paper notes. Do not use
`Sources/Raw/` for paper manifests or paper lifecycle metadata. `Sources/Raw/`
...
- Prefer path-qualified inline wikilinks to wiki-local paper notes, for
  example `[[Sources/Papers/<citekey>|paper alias]]`. Do not use bare citekey
  links for paper provenance unless a validator-visible alias page exists. Use
  source footnotes only when a multi-source paper note needs paragraph-level
...
- For cross-domain conceptual links, use explicit path-qualified wikilinks such
  as `[[Cards/cs/simt-core-pipeline|SIMT core pipeline]]` when
...
Do not write Sources/Raw/, wiki-local Zotero bundles, manifest.md, origin.md, extracted files, paper registries, domain index.md, or domain log.md.
...
operation. Ordinary downstream synthesis writes inside `Cards/<domain>/`,
`Atlas/`, `Sources/`, or `Spaces/` under the root-layout contract and must
treat Zotero-managed paper raw files as read-only sources. Do not write inside
