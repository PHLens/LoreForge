...
- skip `z-Legacy/`, `90-Legacy/`, and other legacy imports as validation targets
...
1. **Raw package integrity**
   - `Sources/Raw/` entries should be source package directories.
   - Each `Sources/Raw/<source-id>/` package must include `origin.md` and
...
2. **Zotero paper note format**
   - Validate existing notes at `Sources/Papers/<citekey>.md`.
...
4. **Links and provenance**
   ...
   - Provenance pointing to non-existent raw manifests or source notes
   - Path-qualified links into `z-Legacy/` or `90-Legacy/` are ignored by
     validation because legacy imports are not active LoreForge pages.

5. **Domain quality**
   - Missing or stale index entries in legacy `Domains/<domain>/` layouts only
   - Orphan pages with no inbound links
   - Pages over 200 lines
   - Tags not listed in `00_System/card-domains.md` for root Card domains or
     legacy `SCHEMA.md` for legacy domains
   - Pages with more than 3 tags
   - `confidence: low`, `contested: true`, or contradiction markers

6. **Write policy and transaction maintenance**
   - Deprecated per-domain `SCHEMA.md`, `index.md`, and `log.md` files under
     root `Cards/<domain>/`
   - High-risk writes without a required transaction snapshot
   - Transaction snapshots past the retention window in `00_System/agent-policy.md`
...
3. Missing legacy index/log/frontmatter or root Card policy
4. Provenance and confidence risks
5. Style and maintenance issues

If the user asked for a write-capable lint pass, apply only explicitly
authorized fixes and rely on validator output for the audit trail. Write a
transaction snapshot only when `00_System/agent-policy.md` marks the operation
high risk. Otherwise keep the check read-only.
