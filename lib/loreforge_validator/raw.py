"""Validation strategy for raw source packages."""
...
def raw_roots_for_wiki(wiki: Path) -> list[Path]:
    return [candidate for candidate in (wiki / "Sources" / "Raw", wiki / "Shared" / "Raw") if candidate.exists()]


def raw_root_for_wiki(wiki: Path) -> Path | None:
    roots = raw_roots_for_wiki(wiki)
...
def validate_raw_packages(wiki: Path) -> list[Issue]:
    wiki = wiki.resolve()
    issues: list[Issue] = []
    raw_roots = raw_roots_for_wiki(wiki)
    if not raw_roots:
        return issues
            ...
            if raw_child.is_file():
                issues.append(
                    Issue(
                        "unexpected-raw-file",
                        rel(raw_child, wiki),
                        f"raw sources must live in {rel(raw_root, wiki)}/<source-id>/ packages",
                    )
                )
